...
class LenientFactualCorrectness(SingleTurnMetric):
    ...
    # Must accept a second argument for run_config
...
df_data = pd.DataFrame([
    {"user_input": "What is the speed?", "response": "6.25 knots", "reference": "6.17 knots"}
])
eval_dataset = EvaluationDataset.from_pandas(df_data)

metric = LenientFactualCorrectness()
results = evaluate(eval_dataset, [metric])
print(results)
